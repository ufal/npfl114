...
class Network:
    ...
    def load(self, path, graph=False):
        if graph:
            if self.session.graph.finalized:
                raise ValueError("Cannot load network into finalized graph.")

            with self.session.graph.as_default():
                self.saver = tf.train.import_meta_graph(path + ".meta")
                public_ops = tf.get_collection("public_ops")
                self.images = public_ops.pop(0)
                self.labels = public_ops.pop(0)
                self.predictions = public_ops.pop(0)
                self.global_step = public_ops.pop(0)
                self.training = public_ops.pop(0)
                self.accuracy = public_ops.pop(0)
                self.summaries = {}
                for dataset in ["training", "dev", "test"]:
                    self.summaries[dataset] = public_ops.pop(0)
    ...
    def save(self, path):
        if not self.saver:
            raise ValueError("Saver is required for saving the model.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp", default="2-mnist-saving-restoring", type=str, help="Experiment name.")
