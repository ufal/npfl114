...
class DiscretizationWrapper(gym.ObservationWrapper):
    def __init__(self, env, separators, tiles=None):
        ...
        if tiles is None:
            ...
            for separator in separators:
                if len(separator) <= 1:
                    self._separator_offsets.append(0)
                    self._separator_tops.append(np.inf)
                else:
                    self._separator_offsets.append((separator[1] - separator[0]) / tiles)
                    self._separator_tops.append(separator[-1] + (separator[1] - separator[0]))
    ...
    class TypedNpFunctionWrapperMethod:
        ...
        def __call__(self, *args, **kwargs):
            ...
            return structural_map(
                np.array, self.__wrapped__(*[np.asarray(arg, typ) for arg, typ in zip(args, types)], **kwargs))
