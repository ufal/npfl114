...
~~~
By a _fundamental theorem of random fields (the Hammersley–Clifford theorem)_, the
...
probability as:
$$P(→y | ⇉X) ∝ \exp\bigg(∑_{i=1}^N \log P(y_i | ⇉X) + ∑_{i=2}^N \log P(y_i, y_{i-1})\bigg).$$
...
$$s(⇉X, →y; →θ, ⇉A) = f_{→θ}(y_1 | ⇉X) + ∑\nolimits_{i=2}^N \big(⇉A_{y_{i-1}, y_i} + f_{→θ}(y_i | ⇉X)\big)$$
~~~
...
$$α_t(k) = f_{→θ}(y_t=k | ⇉X) + \operatorname{logsumexp}_{j∈Y} \big(α_{t-1}(j) + ⇉A_{j, k}\big).$$
...
**Inputs**: Network computing $f_{→θ}(y_t = k | ⇉X)$, which is a logit (unnormalized log-probability)
