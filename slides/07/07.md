...
`tf.keras.layers.Dropout` to force the same mask across timesteps.
The variational dropout on the hidden states can be implemented using
`recurrent_dropout` argument of `tf.keras.layers.{RNN,LSTM,GRU}{,Cell}`.
...
~~~
Note the architecture can be considered as a variant of a mobine inverted
...
![w=63%,h=center](sequence_prediction_training.pdf)

~~~
During inference, use predicted elements as further inputs.

![w=63%,h=center](sequence_prediction_inference.pdf)
...
Technically we do not use one-hot encoding and then a fully connected layer.
