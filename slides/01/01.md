...
![w=46%,f=right](joint_probability.svgz)
...
$$P(⁇x=x_1) = ∑_y P(⁇x = x_1, ⁇y = y).$$

~~~
**Conditional distribution** is a distribution of one (or a subset) of the
random variables, given than another event has already occurred:
$$P(⁇x=x_1 | ⁇y=y_1) = P(⁇x = x_1, ⁇y = y_1) / P(⁇y = y_1).$$
