...
| [UD](http://universaldependencies.org/) | _Universal Dependencies_: Treebanks of 76 languages with consistent annotation of lemmas, POS tags, morphology, syntax. | 183 treebanks |
...
Currently, one of the best architecture is EfficientNet, which combines
...
~~~
- $σ$ (but works badly – nonsymmetrical, repeated application converges to 1,
  $\frac{dσ}{dx}(0) = 1/4$)
...
<br>(Later a proof was given also for $φ = \ReLU$ and even for any nonpolynomial
function.)

For any $ε > 0$ and any continuous function $f$ on $[0, 1]^D$ there exists
an $N ∈ ℕ$, $→v ∈ ℝ^N$, $b ∈ ℝ^N$ and $⇉W ∈ ℝ^{N×D}$, such that if we denote
$$F(→x) = →v^T φ(⇉W→x + →b),$$
