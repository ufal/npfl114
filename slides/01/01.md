...
$P(x)$ or explicitly as $P(⁇x = x)$. All probabilities are nonnegative and sum
...
$H ∈ ℕ$, $→v ∈ ℝ^H$, $→b ∈ ℝ^H$ and $⇉W ∈ ℝ^{H×D}$, such that if we denote
$$F(→x) = →v^T φ(⇉W→x + →b) = ∑_{i=1}^H v_i φ(→W_i^T →x + b_i),$$
