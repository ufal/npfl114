...
Let $φ: ℝ → ℝ$ be a nonconstant, bounded and nondecreasing continuous function. \
...
$N ∈ ℕ$, $→v ∈ ℝ^N$, $b ∈ ℝ^N$ and $⇉W ∈ ℝ^{N×D}$, such that if we denote
$$F(→x) = →v^T φ(⇉W→x + →b),$$
where $φ$ is applied elementwise, then for all $→x ∈ [0, 1]^D$:
$$|F(→x) - f(→x)| < ε.$$
