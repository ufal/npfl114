...
= \argmin\_→θ 𝔼\_{(→x, y)} L(f(→x^; →θ), y^)$, we may use _gradient descent_:
$$→θ ← →θ - α∇\_→θJ(→θ)$$
...
- ReLU:
  $$ \frac{d\operatorname{ReLU}(x)}{dx} = \begin{cases} 1 &\text{if } x ≥ 0 \\\ 0 &\text{if } x ≤ 0 \end{cases}$$
...
- Repeat until stopping criterion is met:
    ...
    - $→θ ← →θ - \frac{α\_t}{\sqrt{\hat→r + ε}}\hat→s$
]
