...
A course intended as a prequel to Deep Learning – introduction to machine
learning (regression, classification, structured prediction, clusterization,
hyperparameter optimization; decision trees, SVM, maximum entropy classifiers,
gradient boosting, …), with practicals in Python.
...
vectors $→r_{t-1}^1, …, →r_{t-1}^R$ from previous time step, and output of the
