...
proposed by Peter et al., called ELMo, **E**mbeddings from **L**anguage
**Mo**dels.
...
~~~
Dropout stochastically multiplies the input by zero.
~~~
Zoneout is an alternative to dropout, stochastically multiplying the input by
one instead of performing a computation.

~~~
![w=40%,f=right](gelu.pdf)

All these functionalities are merged in Gaussian error linear units (GELUs),
...
The Multilingual BERT is pre-trained on 102 largest Wikipedias, including the
Czech one.
